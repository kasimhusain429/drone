...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    // ==========================================
    // DESKTOP ANIMATION (Screens 768px and up)
    // ==========================================
    mm.add("(min-width: 768px)", () => {
      ...
      gsap.set(canvasWrapperRef.current, { x: '15vw', y: 0, scale: 0.7 });
    ...
    // ==========================================
    // MOBILE ANIMATION (Screens below 768px)
    // ==========================================
    mm.add("(max-width: 767px)", () => {
      // On mobile, object-contain is used, so scale 1.0 = full width of screen.
      // Drone starts high up (y: -25vh) and scale 1.0 to perfectly avoid the text block below.
      gsap.set(canvasWrapperRef.current, { x: 0, y: '-25vh', scale: 1.0 });
      ...
      // Canvas moves to dead center (y: 0) to fit inside the gap-[30vh] of the glass card.
      tl.to(canvasWrapperRef.current, { y: 0, scale: 1.0, duration: 2.5, ease: "power2.inOut" }, 1);

      // Glass Card Catch (15% - 45%)
      tl.to(cardRef.current, { scale: 1, opacity: 1, duration: 2, ease: "back.out(1.2)" }, 1.5);
      tl.fromTo(cardLeftRef.current, { y: -50 }, { y: 0, opacity: 1, duration: 1 }, 2);
      tl.fromTo(cardRightRef.current, { y: 50 }, { y: 0, opacity: 1, duration: 1 }, 2);
      tl.fromTo(cardBottomRef.current, { y: 20 }, { y: 0, opacity: 1, duration: 1 }, 2.5);

      // --- Phase 2 -> Phase 3 Transition (45% - 60%) ---
      tl.to(cardRef.current, { xPercent: -150, duration: 1.5, ease: "power2.inOut" }, 4.5);
      
      // Canvas moves up again to make room for carousel below it
      tl.to(canvasWrapperRef.current, { y: '-25vh', scale: 1.0, duration: 1.5, ease: "power2.inOut" }, 4.5);
      ...
      // Canvas returns to center for final CTA
      tl.to(canvasWrapperRef.current, { y: 0, scale: 1.0, duration: 1.5, ease: "power2.inOut" }, 7.5);
  ...
  return (
    <main ref={containerRef} className="relative h-[400vh] text-white font-sans antialiased overflow-x-hidden selection:bg-lime-400 selection:text-black">
      ...
      <div className="fixed inset-0 z-10 pointer-events-none">
        ...
        {/* Phase 1: Hero */}
        <div className="absolute inset-0 flex flex-col justify-end md:justify-center items-center md:items-start pb-8 md:pb-0 px-8 md:px-24">
          <div ref={heroRef} className="max-w-[400px] pointer-events-auto text-center md:text-left mx-auto md:mx-0">
            <div className="text-[10px] md:text-xs font-bold tracking-[0.3em] text-neutral-400 mb-2 md:mb-4 uppercase">Premium Drone Services</div>
            ...
            <div className="flex gap-3 md:gap-4 flex-col md:flex-row w-full md:w-auto">
              <button className="w-full md:w-auto px-8 py-3 bg-lime-400 text-black font-semibold rounded-full hover:bg-lime-500 transition shadow-[0_0_20px_rgba(163,230,53,0.3)]">
                View Services
              </button>
              <button className="w-full md:w-auto px-8 py-3 bg-transparent text-current border border-current/30 font-semibold rounded-full hover:bg-white/10 transition">
                Contact Us
              </button>
            </div>
          </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div 
            ref={cardRef} 
            className="w-[90vw] md:w-[90vw] max-w-[1200px] h-[85vh] md:h-[70vh] bg-[#191919]/60 backdrop-blur-[20px] border border-white/10 rounded-[40px] shadow-2xl flex flex-col justify-between p-8 md:p-12 pointer-events-auto will-change-transform"
          >
            ...
            <div className="flex flex-col md:flex-row justify-between items-center w-full mt-8 md:mt-8 text-white gap-[30vh] md:gap-0">
              {/* Reduced width to 25% on desktop to leave massive 50% gap in center for the drone */}
              <div ref={cardLeftRef} className="w-full md:w-[25%] text-center md:text-left">
              ...
              <div ref={cardRightRef} className="w-full md:w-[25%] text-center md:text-right">
                <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-2 md:mb-4">Construction</h2>
                <p className="text-neutral-400 text-sm md:text-base">Clear views for your stakeholders.</p>
              </div>
            </div>
            
            <div ref={cardBottomRef} className="flex flex-col md:flex-row justify-between items-center md:items-end text-white gap-6 md:gap-0 mt-8 md:mt-0 text-center md:text-left">
        ...
        {/* Phase 3: Carousel */}
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-12 md:pb-24">
          <h2 ref={carouselTitleRef} className="text-3xl md:text-5xl font-bold text-black mb-8 md:mb-12 tracking-tight text-center px-4">Premium Quality Imagery</h2>
          
          <div ref={carouselWrapperRef} className="w-full pl-8 md:pl-[45vw] pr-8 md:pr-12 flex flex-row gap-4 md:gap-8 overflow-visible will-change-transform pointer-events-auto">
            <div className="flex-shrink-0 w-[280px] md:w-[400px]">
              <img src="/images/roofing_inspection_1789981969219.jpg" alt="Roofing" className="w-full h-[200px] md:h-[300px] object-cover rounded-2xl md:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-4 border-white" />
            </div>
            <div className="flex-shrink-0 w-[280px] md:w-[400px]">
              <img src="/images/solar_inspection_1789981984532.jpg" alt="Solar" className="w-full h-[200px] md:h-[300px] object-cover rounded-2xl md:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-4 border-white md:mt-12" />
            </div>
            <div className="flex-shrink-0 w-[300px] md:w-[500px]">
              <img src="/images/construction_progress_1789982002449.jpg" alt="Construction" className="w-full h-[200px] md:h-[300px] object-cover rounded-2xl md:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-4 border-white md:-mt-8" />
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div ref={ctaRef} className="flex flex-col items-center will-change-transform pointer-events-auto">
            <h2 className="text-[18vw] md:text-[12vw] font-black text-black tracking-tighter leading-none mb-12">Dronefuze</h2>
            <button className="px-8 md:px-12 py-4 md:py-5 bg-[#0a0a0a] text-white text-lg md:text-xl font-bold rounded-full hover:bg-neutral-800 transition hover:scale-105 transform shadow-xl flex items-center gap-3">
              Book an Inspection
